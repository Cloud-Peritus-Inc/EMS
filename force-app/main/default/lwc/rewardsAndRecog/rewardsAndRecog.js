...
@track name;
   @wire(getTheCurrentRRTrends) 
    wiredRR({error, data}){
        ...
        //smaske :[UAT_076] : Removing HTML tags from awardComments as the data was retrieved from RichTextArea Field
        //this.currentdata = data.currentList;
        this.currentdata = data.currentList.map(rrcurrent => ({
            ...rrcurrent,
            sanitizedAwardComments: this.removeHTMLTags(rrcurrent.awardComments)
        }));
    ...
    //smaske :[UAT_076] : Removing HTML tags from RichText awardComments
    removeHTMLTags(html) {
