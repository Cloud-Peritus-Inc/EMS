...
export default class ServiceRequestLeaveTypesForm extends NavigationMixin(LightningElement) {
    ...
    splLeaveID;
    handleSuccess(event) {
        this.splLeaveID = event.detail.id;
        //console.log('splLeaveID--------->' , this.splLeaveID);
        if (this.fileData != null) {
                uploadFile({ base64: this.fileData.base64, filename: this.fileData.filename, recordId: this.splLeaveID }).then(res => {
                    //console.log(res);
    ...
    submitcase(event) {
        ...
        //smaske : [SR__002] : 23-OCT-2024 : updating <56 with <=56 and moving File Upload validation outside the createRecord Method.
        if (diffDays <= 56) { 
            const evt = new ShowToastEvent({
