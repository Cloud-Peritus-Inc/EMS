...
export default class TimesheetComponent2 extends  NavigationMixin(LightningElement) {
    ...
    */
    initialValues() {
        ...
        this.records = [{key: 0, otherTask: false, EMS_TM_Sun__c: 0, EMS_TM_Mon__c: 0, EMS_TM_Tue__c: 0, EMS_TM_Wed__c: 0, EMS_TM_Thu__c: 0, EMS_TM_Fri__c: 0, EMS_TM_Sat__c: 0, projectTaskOptions: [], projectName: ''}];
    }
    ...
    calculateTotalHours() {
        ...
        for( let key in this.totalDayHours) {
            if (this.totalDayHours[key] && key.length === 13 && (key != 'EMS_TM_Sat__c' && key != 'EMS_TM_Sun__c')) {
                let error = 'error'+key;
                if (this.totalDayHours[key] > 24 || this.totalDayHours[key] < 0 ) { /*changing here from 8 to 0 PT for Validation input */
