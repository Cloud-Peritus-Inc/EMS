...
export default class LightningExampleAccordionMultiple extends LightningElement {
  ...
  handleFileUploaded(event) {
    ...
    const MAX_INDIVIDUAL_FILE_SIZE = 3 * 1024 * 1024; // 3 MB
    ...
    if (event.target.files.length > 0) {
      ...
      for (let i = 0; i < event.target.files.length; i++) {
        ...
        if (file.size > MAX_INDIVIDUAL_FILE_SIZE) {
          this.showToast('error', 'File size exceeded the individual file size limit.');
          return;
        }
        ...
        if (totalFileSize > MAX_TOTAL_FILE_SIZE) {
          this.showToast('error', 'Total file size exceeded the upload size limit.');
          return;
        }
