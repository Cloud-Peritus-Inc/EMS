...
import deleteSelectedfile from "@salesforce/apex/EMS_EM_CreationOnboard.deleteSelectedfile";
import { refreshApex } from '@salesforce/apex';
import getPicklistValues from '@salesforce/apex/EMS_EM_CreationOnboard.getPicklistValues';
...
export default class LightningExampleAccordionMultiple extends LightningElement {
  ...
  @wire(getPicklistValues, { objectName: 'EMS_EM_Onboarding_Request__c', fieldName: 'CA_Country__c' })
  ...
  AddressCheckboxChange(event) {
    ...
    if (this.inputcheckboxValue == 'Checked') {
      ...
      this.countryvalpa = this.countryval;
      this.paFlag = true;
  ...
  handleChange(event) {
    ...
    if (field === 'PA_Country__c') {
      this.countryvalpa = event.target.value;
      if (event.target.value == '') {
        this.countryvalpa = null;
      }
      if (this.inputcheckboxValue == 'Checked') {
        this.countryval = this.countryvalpa;
      }
    }
  }
  ...
  handleSaveEduAction() {
    ...
    const isInputsCorrect = [...this.template.querySelectorAll('lightning-input, lightning-combobox')]
      ...
      /* let totalFileSize = this.filesData.reduce((total, fileData) => total + fileData.fileContent.length, 0);
      console.log('totalFileSize-' ,totalFileSize); */

    if (isInputsCorrect) {
       // Validate file sizes
    let totalFileSize = 0;
    const MAX_INDIVIDUAL_FILE_SIZE = 4187988; // 3 MB
    const MAX_TOTAL_FILE_SIZE = 4187988; // 3 MB

    for (let i = 0; i < this.filesData.length; i++) {
      const file = this.filesData[i];

      if (file.fileContent.length > MAX_INDIVIDUAL_FILE_SIZE) {
        this.handleIsLoading(false);
        this.dispatchEvent(
          new ShowToastEvent({
            message: 'Your file size exceeds 3 MB. Please upload a file of a lower size.',
            variant: 'error',
          }),
        );
        return;
      ...
      totalFileSize += file.fileContent.length;
    }
    ...
    if (totalFileSize > MAX_TOTAL_FILE_SIZE) {
      ...
      this.dispatchEvent(
        new ShowToastEvent({
          message: 'Your Total file size exceeds 3 MB. Please upload a file of a lower size.',
          variant: 'error',
        }),
      );
      return;
      ...
      uploadFiles({ recordId: this.onboardingformId, filedata: JSON.stringify(this.filesData) })
        .then(result => {
          if (result && result == 'success') {
            //updateOnboardingInfoOnPageLoads(this);
            this.updateRecordView11(updateOnboardingInfoOnPageLoads(this));
            this.filesData = [];
            //updateOnboardingInfoOnPageLoads(this);
  ...
  handleFileUploaded(event) {
    const MAX_TOTAL_FILE_SIZE = 3145728; // 3 MB
    const MAX_INDIVIDUAL_FILE_SIZE = 3145728; // 3 MB
    ...
    if (event.target.files.length > 0) {
      ...
      for (let i = 0; i < event.target.files.length; i++) {
        ...
        if (file.size > MAX_INDIVIDUAL_FILE_SIZE) {
          this.showToast('error', 'Your file size exceeds 3 MB. Please upload a file of a lower size.');
          return;
        }
  
        if (file.type !== 'application/pdf' && file.name.split('.').pop().toLowerCase() !== 'zip') {
          this.showToast('error', 'Only PDF and Zip file formats are allowed.');
          return;
        }
  
        totalFileSize += file.size;
        console.log('totalFileSize- onchange' ,totalFileSize);
  
        if (totalFileSize > MAX_TOTAL_FILE_SIZE) {
          this.showToast('error', 'Your Total file size exceeds 3 MB. Please upload a file of a lower size.');
          return;
        }
 ...
 */    if (this.workDetails.length < 1) {
  ...
  handleSaveWorkAction() {
    ...
    for (let i = 0; i < this.filesDocData.length; i++) {
      const file = this.filesDocData[i];
  ...
  deleteSelectedRecords(event) {
    ...
    //console.log('recordId set: ', recordId);
  
    deleteSelectedfile({ docIds: recordId })
      .then(result => {
        this.handleIsLoading(false);
        updateOnboardingInfoOnPageLoads(this);
        this.dispatchEvent(
          new ShowToastEvent({
            message: 'Selected file is deleted!',
            variant: 'success',
          }),
        );updateOnboardingInfoOnPageLoads(this);
      })
